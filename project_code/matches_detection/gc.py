...
def start_gc_task(bot, DB_PASSWORD):
    @tasks.loop(minutes=1)
    async def check_upcoming_matches_gc():
        now = datetime.now()
        ...
        query = """
            ...
            FROM matches
            WHERE match2opponents @> '[{"teamtemplate": {"name": "Karmine Corp GC"}}]'
            ...
            with psycopg.connect(f"dbname={dbname} user={user} password={DB_PASSWORD} host={host}") as conn:
                with conn.cursor() as cur:
                    ...
                    for match in matches:
                        ...
                        ennemy_team = [team for team in teams if team != "Karmine Corp GC"][0]
    ...
    if not check_upcoming_matches_gc.is_running():
        check_upcoming_matches_gc.start()