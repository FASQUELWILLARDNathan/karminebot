...
def start_lfl_task(bot, DB_PASSWORD):
    ...
    async def check_upcoming_matches_lfl():
        ...
        query = """
            ...
            FROM matches
            WHERE match2opponents @> '[{"teamtemplate": {"name": "Karmine Corp Blue"}}]'
            ...
            with psycopg.connect(f"dbname={dbname} user={user} password={DB_PASSWORD} host={host}") as conn:
                with conn.cursor() as cur:
                    ...
                    for match in matches:
                        ...
                        # Ajouter 2 heures à la date extraite (assure-toi que 'date' est un objet datetime)
                        date = date + timedelta(hours=2)

                        # Extraire les noms d'équipes directement depuis le JSON
                        teams = [team["teamtemplate"]["name"] for team in match2opponents]
                        ennemy_team = [team for team in teams if team != "Karmine Corp Blue"][0]
    ...
    if not check_upcoming_matches_lfl.is_running():
        check_upcoming_matches_lfl.start()