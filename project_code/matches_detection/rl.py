...
def start_rl_task(bot, DB_PASSWORD):
    @tasks.loop(minutes=1)
    async def check_upcoming_matches_rl():
        now = datetime.now()
        ...
        query = """
            ...
            AND winner = ''
            AND series = 'Rocket League Championship Series'
            ORDER BY date ASC
        """
    ...
    if not check_upcoming_matches_rl.is_running():
        check_upcoming_matches_rl.start()