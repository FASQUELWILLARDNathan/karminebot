...
def start_france_task(bot, DB_PASSWORD):
    @tasks.loop(minutes=1)
    async def check_upcoming_matches_france():
        now = datetime.now()
        ...
        query = """
            ...
            FROM matches
            WHERE match2opponents @> '[{"teamtemplate": {"name": "France"}}]'
            AND date BETWEEN %s AND %s
            AND winner = ''
            AND series = 'FIFAe World Cup'
            ORDER BY date ASC
        """
            ...
            with psycopg.connect(f"dbname={dbname} user={user} password={DB_PASSWORD} host={host}") as conn:
                with conn.cursor() as cur:
                    ...
                    for match in matches:
                        ...
                        ennemy_team = [team for team in teams if team != "France"][0]
    ...
    if not check_upcoming_matches_france.is_running():
        check_upcoming_matches_france.start()