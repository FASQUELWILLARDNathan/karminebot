...
def start_inter_task(bot, DB_PASSWORD):
    ...
    async def check_upcoming_matches_inter_main():
        ...
        query = """
            ...
            FROM matches
            WHERE match2opponents @> '[{"teamtemplate": {"name": "Karmine Corp"}}]'
            AND date BETWEEN %s AND %s
            AND winner = ''
            AND series NOT IN ('VALORANT Champions Tour', 'LEC', 'Rocket League Championship Series', 'VALORANT Challengers Leagues', 'LFL')
            ORDER BY date ASC
        """
            ...
            with psycopg.connect(f"dbname={dbname} user={user} password={DB_PASSWORD} host={host}") as conn:
                with conn.cursor() as cur:
                    ...
                    for match in matches:
                        ...
                        ennemy_team = [team for team in teams if team != "Karmine Corp"][0]
    ...
    if not check_upcoming_matches_inter_main.is_running():
        check_upcoming_matches_inter_main.start()