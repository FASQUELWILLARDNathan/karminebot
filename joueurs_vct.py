...
# Liste noire des membres du staff à exclure
staff_members = ["Trexx", "Magnum", "RobbieBk", "Hype"]
...
params = {
    'wiki': 'valorant',
    'conditions': '([[teampagename::Karmine_Corp]] OR [[teampagename::Team_Liquid]] OR [[teampagename::Natus_Vincere]] OR [[teampagename::Gentle_Mates]] OR [[teampagename::KOI]] OR [[teampagename::BBL_Esports]] OR [[teampagename::Apeks]] OR [[teampagename::GIANTX]] OR [[teampagename::Team_Vitality]] OR [[teampagename::Fnatic]] OR [[teampagename::FUT_Esports]] OR [[teampagename::Team_Heretics]]) AND [[type::player]] AND [[status::Active]] AND [[pagename::!Trexx]] AND [[pagename::!Magnum]] AND [[pagename::!RobbieBk]] AND [[pagename::!Hype]]',
    'limit': 60,
    'offset': 0
}
        ...
        if response.status_code == 200:
            ...
            for player in data:
                if player.get("type") == "player":
                    player_name = player.get("namepage")
                    if player_name not in staff_members:
    ...
    with psycopg.connect(f"dbname={dbname} user={user} password={DB_PASSWORD} host={host}") as conn:
        with conn.cursor() as cursor:
            for team_name, players in all_players.items():
                players_to_insert = players[:5]  # Ne garder que les 5 premiers joueurs
